        ...
        {table.getHeaderGroups().map((headerGroup) => (
          <TableRow key={headerGroup.id}>
            {headerGroup.headers.map((header) => {
              ...
              const sort = header.column.getIsSorted() as
                | false
                | "asc"
                | "desc";
              const ariaSort =
...
export function ProductsTable({ products }: { products: ProductList }) {
...
export default function ProductsTableWithCustomText({ products }: { products: ProductList }) {
  const memo = React.useMemo(() => ProductTableColumn(), []);
  return (
    <TableProvider products={products} cols={memo}>
      <div className="flex flex-col gap-2 p-2">
        <ProductsTableToolbar />
        <div>
            <p className="text-md font-semibold">Showing {products.products.length} products</p>
        </div>
        <TableInner />
        <Pagination />
      </div>
